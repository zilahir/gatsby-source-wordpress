...
function handleQueue(_x, _x2) {
  return _handleQueue.apply(this, arguments);
}
/**
 * @typedef {Options}
 * @type {Object}
...
function _handleQueue() {
  _handleQueue = (0, _asyncToGenerator2.default)(function* (task, cb) {
    try {
      const response = yield request(task);
      cb(null, response);
    } catch (err) {
      cb(err);
    }
...
module.exports = function requestInQueue(tasks, opts = {}) {
  return new Promise((res, rej) => {
    const q = new Queue(handleQueue, Object.assign({}, _defaults, opts));
